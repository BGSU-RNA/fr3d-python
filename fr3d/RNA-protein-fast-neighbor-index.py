...
def find_neighbors(bases, amino_acids, aa_part, dist_cent_cutoff):
    ...
    aaCubeList = {}
    for a in range(0,len(amino_acids)):
        center = amino_acids[a].centers[aa_part]
        x = floor(center[0]/dist_cent_cutoff)
        y = floor(center[1]/dist_cent_cutoff)
        z = floor(center[2]/dist_cent_cutoff)
        key = "%d,%d,%d" % (x,y,z)
        if key in aaCubeList:
            aaCubeList[key].append(a)
        else:
            aaCubeList[key] = [a]
    ...
    print "Number of baseCubes",len(baseCubeList)
    print "Number of aaCubes",len(aaCubeList)

    start = datetime.now()
    for key in baseCubeList:
        for aakey in baseCubeNeighbors[key]:
            if aakey in aaCubeList:
                for b in baseCubeList[key]:
                    base_residue = bases[b]
                    ...
                    for a in aaCubeList[aakey]:
                        aa_residue = amino_acids[a]
...
if __name__=="__main__":
    for PDB in PDB_List:
        ...
        start = datetime.now()
        list_base_aa, list_aa_coord, list_base_coord = find_neighbors(list(bases), list(amino_acids), aa_part, 10)
